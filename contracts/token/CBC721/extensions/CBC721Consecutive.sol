...
// OpenZeppelin Contracts (last updated v5.5.0) (token/CBC721/extensions/CBC721Consecutive.sol)

pragma solidity ^0.8.24;

import {CBC721} from "../CBC721.sol";
import {ICBC2309} from "../../../interfaces/ICBC2309.sol";
import {BitMaps} from "../../../utils/structs/BitMaps.sol";
import {Checkpoints} from "../../../utils/structs/Checkpoints.sol";

/**
 * @dev Implementation of the CBC-2309 "Consecutive Transfer Extension" as defined in
 * https://eips.ethereum.org/EIPS/eip-2309[CBC-2309].
 *
 ...
 */
abstract contract CBC721Consecutive is ICBC2309, CBC721 {
     ...
     * Any batch mint not emitting the {ICBC721-Transfer} event outside of the constructor
     * is non CBC-721 compliant.
     */
    error CBC721ForbiddenBatchMint();

    /**
     * @dev Exceeds the max amount of mints per batch.
     */
    error CBC721ExceededMaxBatchMint(uint256 batchSize, uint256 maxBatch);

    /**
     * @dev Individual minting is not allowed.
     */
    error CBC721ForbiddenMint();

    /**
     * @dev Batch burn is not supported.
     */
    error CBC721ForbiddenBatchBurn();
    ...
    /**
     * @dev See {CBC721-_ownerOf}. Override that checks the sequential ownership structure for tokens that have
     ...
     *
     * CAUTION: Does not emit a `Transfer` event. This is CBC-721 compliant as long as it is done inside of the
     * constructor, which is enforced by this function.
     *
     * CAUTION: Does not invoke `onCBC721Received` on the receiver.
     *
     * Emits a {ICBC2309-ConsecutiveTransfer} event.
     */
    function _mintConsecutive(address to, uint96 batchSize) internal virtual returns (uint96) {
        ...
        if (batchSize > 0) {
            if (address(this).code.length > 0) {
                revert CBC721ForbiddenBatchMint();
            }
            if (to == address(0)) {
                revert CBC721InvalidReceiver(address(0));
            }

            uint256 maxBatchSize = _maxBatchSize();
            if (batchSize > maxBatchSize) {
                revert CBC721ExceededMaxBatchMint(batchSize, maxBatchSize);
            }
    ...
    /**
     * @dev See {CBC721-_update}. Override version that restricts normal minting to after construction.
     *
     * WARNING: Using {CBC721Consecutive} prevents minting during construction in favor of {_mintConsecutive}.
