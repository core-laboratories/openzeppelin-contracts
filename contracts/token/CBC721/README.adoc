...
This set of interfaces, contracts, and utilities is all related to the https://eips.ethereum.org/EIPS/eip-721[CBC-721 Non-Fungible Token Standard].

TIP: For a walk through on how to create an CBC-721 token read our xref:ROOT:erc721.adoc[CBC-721 guide].

The CBC specifies four interfaces:
...
* {ERC721Consecutive}: An implementation of https://eips.ethereum.org/EIPS/eip-2309[CBC-2309] for minting batches of tokens during construction, in accordance with CBC-721.
...
* {ERC721Royalty}: A way to signal royalty information following CBC-2981.
* {ERC721Pausable}: A primitive to pause contract operation.
* {ERC721Burnable}: A way for token holders to burn their own tokens.
* {ERC721Wrapper}: Wrapper to create an CBC-721 backed by another CBC-721, with deposit and withdraw methods. Useful in conjunction with {ERC721Votes}.

NOTE: This core set of contracts is designed to be unopinionated, allowing developers to access the internal functions in CBC-721 (such as <<ERC721-_mint-address-uint256-,`_mint`>>) and expose them as external functions in the way they prefer.
