...
// OpenZeppelin Contracts (last updated v5.3.0) (token/CBC20/extensions/draft-CBC20TemporaryApproval.sol)

pragma solidity ^0.8.24;

import {ICBC20, CBC20} from "../CBC20.sol";
import {ICBC7674} from "../../../interfaces/draft-ICBC7674.sol";
import {Math} from "../../../utils/math/Math.sol";
import {SlotDerivation} from "../../../utils/SlotDerivation.sol";
import {TransientSlot} from "../../../utils/TransientSlot.sol";

/**
 * @dev Extension of {CBC20} that adds support for temporary allowances following CBC-7674.
 *
 * WARNING: This is a draft contract. The corresponding CBC is still subject to changes.
 *
 * _Available since v5.1._
 */
abstract contract CBC20TemporaryApproval is CBC20, ICBC7674 {
    using SlotDerivation for bytes32;
    using TransientSlot for bytes32;
    using TransientSlot for TransientSlot.Uint256Slot;

    // keccak256(abi.encode(uint256(keccak256("openzeppelin.storage.CBC20_TEMPORARY_APPROVAL_STORAGE")) - 1)) & ~bytes32(uint256(0xff))
    bytes32 private constant CBC20_TEMPORARY_APPROVAL_STORAGE =
     ...
     */
    function allowance(address owner, address spender) public view virtual override(ICBC20, CBC20) returns (uint256) {
    ...
    function _temporaryApprove(address owner, address spender, uint256 value) internal virtual {
        if (owner == address(0)) {
            revert CBC20InvalidApprover(address(0));
        }
        if (spender == address(0)) {
            revert CBC20InvalidSpender(address(0));
        }
    ...
    function _temporaryAllowanceSlot(address owner, address spender) private pure returns (TransientSlot.Uint256Slot) {
        return CBC20_TEMPORARY_APPROVAL_STORAGE.deriveMapping(owner).deriveMapping(spender).asUint256();
    }
}
