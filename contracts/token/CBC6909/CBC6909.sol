...
// OpenZeppelin Contracts (last updated v5.5.0) (token/CBC6909/CBC6909.sol)

pragma solidity ^0.8.20;

import {ICBC6909} from "../../interfaces/ICBC6909.sol";
 ...
 */
contract CBC6909 is Context, CBC165, ICBC6909 {
    ...
    error CBC6909InsufficientBalance(address sender, uint256 balance, uint256 needed, uint256 id);
    error CBC6909InsufficientAllowance(address spender, uint256 allowance, uint256 needed, uint256 id);
    error CBC6909InvalidApprover(address approver);
    error CBC6909InvalidReceiver(address receiver);
    error CBC6909InvalidSender(address sender);
    error CBC6909InvalidSpender(address spender);

    /// @inheritdoc ICBC165
    function supportsInterface(bytes4 interfaceId) public view virtual override(CBC165, ICBC165) returns (bool) {
        return interfaceId == type(ICBC6909).interfaceId || super.supportsInterface(interfaceId);
    }
    ...
    function _mint(address to, uint256 id, uint256 amount) internal {
        if (to == address(0)) {
            revert CBC6909InvalidReceiver(address(0));
        }
    ...
    function _transfer(address from, address to, uint256 id, uint256 amount) internal {
        if (from == address(0)) {
            revert CBC6909InvalidSender(address(0));
        }
        if (to == address(0)) {
            revert CBC6909InvalidReceiver(address(0));
        }
    ...
    function _update(address from, address to, uint256 id, uint256 amount) internal virtual {
        ...
        if (from != address(0)) {
            ...
            if (fromBalance < amount) {
                revert CBC6909InsufficientBalance(from, fromBalance, amount, id);
            }
            unchecked {
    ...
    function _approve(address owner, address spender, uint256 id, uint256 amount) internal virtual {
        if (owner == address(0)) {
            revert CBC6909InvalidApprover(address(0));
        }
        if (spender == address(0)) {
            revert CBC6909InvalidSpender(address(0));
        }
    ...
    function _spendAllowance(address owner, address spender, uint256 id, uint256 amount) internal virtual {
        ...
        if (currentAllowance < type(uint256).max) {
            if (currentAllowance < amount) {
                revert CBC6909InsufficientAllowance(spender, currentAllowance, amount, id);
            }
            unchecked {
