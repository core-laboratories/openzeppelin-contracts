 ...
 * contract.
 *
 * {ICBC165} interfaces can also be queried via the registry.
 *
 * For an in-depth explanation and source code analysis, see the ERC text.
 */
interface ICBC1820Registry {
     ...
     *
     * - the caller must be the current manager for `account`.
     * - `interfaceHash` must not be an {ICBC165} interface id (i.e. it must not
     * end in 28 zeroes).
     ...
     *
     * If `interfaceHash` is an {ICBC165} interface id (i.e. it ends with 28
     ...
     */
    function updateCBC165Cache(address account, bytes4 interfaceId) external;
     ...
     */
    function implementsCBC165Interface(address account, bytes4 interfaceId) external view returns (bool);
     ...
     * @return True if `account` implements `interfaceId`, false otherwise.
     */
    function implementsCBC165InterfaceNoCache(address account, bytes4 interfaceId) external view returns (bool);
}
