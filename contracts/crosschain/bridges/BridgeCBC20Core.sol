 ...
 *
 * This base contract is used by the {BridgeCBC20}, which interfaces with legacy ERC-20 tokens, and {BridgeERC7802},
 * which interface with ERC-7802 to provide an approve-free user experience. It is also used by the {CBC20Crosschain}
 * extension, which embeds the bridge logic directly in the token contract.
 */
abstract contract BridgeCBC20Core is Context, CrosschainLinked {
    using InteroperableAddress for bytes;

    event CrosschainCBC20TransferSent(bytes32 indexed sendId, address indexed from, bytes to, uint256 amount);
    event CrosschainCBC20TransferReceived(bytes32 indexed receiveId, bytes from, address indexed to, uint256 amount);
    ...
    function _crosschainTransfer(address from, bytes memory to, uint256 amount) internal virtual returns (bytes32) {
        ...
        emit CrosschainCBC20TransferSent(sendId, from, to, amount);
    ...
    function _processMessage(
        ...
        emit CrosschainCBC20TransferReceived(receiveId, from, to, amount);
    }
